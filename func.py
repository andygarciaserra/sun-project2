...
def Img_calibration(DATADIR, plot):
    ...
    print('Minimum 1 in fit: '+str(x_min1_surr[min1fit_pos]))
    min2fit_pos = Find_minimum(min2_fit,x_min2_surr)
    print('Minimum 2 in fit: '+str(x_min2_surr[min2fit_pos]))
    ...
    print('Minimum 1 in atlas: '+str(atlas[min1atlas_pos,0]))
    print('Minimum 2 in atlas: '+str(atlas[min2atlas_pos,0]))

    #Final calibration
    curve = Calibration_Curve([atlas[]],[])
                # Me falta encontrar los mínimos en hinode y con esos mínimos
                # y los del fit hacer la curva de calibración
